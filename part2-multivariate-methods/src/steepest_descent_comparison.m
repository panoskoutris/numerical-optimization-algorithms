...
max_iter = 150;  
tol = 10^(-3);  % Tolerance
sigma = 0.5;  % Constant for the Armijo rule

% Run steepest descent with different step choices
[x_history_1, f_history_1] = steepest_descent(f, grad_f, x0, 1, max_iter, tol, sigma);  % Fixed step
[x_history_2, f_history_2] = steepest_descent(f, grad_f, x0, 2, max_iter, tol, sigma);  % Line search
[x_history_3, f_history_3] = steepest_descent(f, grad_f, x0, 3, max_iter, tol, sigma);  % Armijo rule
...
sgtitle('Convergence of Function Value Across Methods (Deepest Descent)');  